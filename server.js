...
io.on('connection',socket=>{
 ...
 // The callee must explicitly accept before an SDP offer is sent.
 socket.on('call:invite',async p=>{
   const to=Number(p?.to); if(!to||to===socket.userId)return;
   const target=(await db('SELECT id,display_name,avatar_url,is_disabled FROM users WHERE id=$1',[to]))[0];
   if(!target||target.is_disabled)return;
   io.to('user:'+to).emit('call:invite',{from:socket.userId,fromName:socket.user.displayName,fromAvatar:socket.user.avatarUrl,callType:p.callType==='video'?'video':'voice',callId:String(p.callId)});
 });
 socket.on('call:accept',p=>{const to=Number(p?.to);if(to)io.to('user:'+to).emit('call:accepted',{from:socket.userId,callId:String(p.callId)});});
 socket.on('call:reject',p=>{const to=Number(p?.to);if(to)io.to('user:'+to).emit('call:rejected',{from:socket.userId,callId:String(p.callId),reason:p.reason||'rejected'});});
 socket.on('call:busy',p=>{const to=Number(p?.to);if(to)io.to('user:'+to).emit('call:rejected',{from:socket.userId,callId:String(p.callId),reason:'busy'});});
 socket.on('call:offer',p=>{const to=Number(p?.to);if(to)io.to('user:'+to).emit('call:offer',{from:socket.userId,fromName:socket.user.displayName,callType:p.callType,offer:p.offer,callId:String(p.callId)});});
